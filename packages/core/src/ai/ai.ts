...
// AI configuration is now imported from ../core/types
    ...
    try {
      switch (this.config.provider) {
        ...
        case 'azure': {
          ...
          const azureEndpoint = this.config.apiEndpoint || 'https://YOUR_RESOURCE.openai.azure.com';
          this.client = {
            provider: 'azure',
            config: this.config,
        ...
        case 'ollama': {
          ...
          this.client = {
            provider: 'ollama',
            endpoint: this.config.apiEndpoint || 'http://localhost:11434',
            config: this.config,
          };
          break;
        }
