...
describe('StdioTransport', () => {
    ...
    describe('Basic Functionality', () => {
        ...
        test('should spawn child process with correct arguments when connecting', async () => {
            ...
            expect(childProcess.spawn).toHaveBeenCalledWith('test-server', ['--verbose', '--port', '8080'], expect.any(Object));
        });
    ...
    describe('Connection Management', () => {
        test('should connect successfully', async () => {
            ...
            const readyCallback = mockChildProcess.on.mock.calls.find((call) => call[0] === 'spawn')?.[1];
            if (readyCallback) {
                readyCallback();
            }
            // Act
        ...
        test('should disconnect successfully', async () => {
            ...
            transport.process = mockChildProcess;
            // Act
            await transport.disconnect();
            // Assert
            expect(mockChildProcess.kill).toHaveBeenCalled();
            expect(transport.connected).toBe(false);
            expect(transport.process).toBeUndefined();
        });
    ...
    describe('Message Handling', () => {
        ...
        test('should send JSON-RPC request', async () => {
            ...
            // Assert
            expect(mockChildProcess.stdin.write).toHaveBeenCalledWith(JSON.stringify(request) + '\n', expect.any(Function));
        });
        ...
        test('should handle malformed JSON gracefully', () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('[MCP Transport] JSON-like message but cannot parse'));
            // Cleanup
    ...
    describe('Configuration Options', () => {
        ...
        test('should create transport with working directory', async () => {
            ...
            const cwdSpawnCall = spawnCalls.find((call) => call[0] === 'test-server' && call[2]?.cwd === '/tmp/test-dir');
        ...
        test('should create transport with log filter configuration', () => {
            ...
            expect(logFilterTransport.config.logFilter).toEqual(logFilterConfig.logFilter);
        });
    });
});
