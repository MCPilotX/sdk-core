...
import { FallbackManager, FallbackStrategy, FallbackOptions, defaultFallbackManager } from '../src/mcp/fallback-manager';
import { ToolCall, ToolResult } from '../src/mcp/types';
...
describe('FallbackManager', () => {
  ...
  describe('Private Methods (indirect testing)', () => {
    describe('extractIntentFromToolName', () => {
      it('should extract intent from tool names', () => {
        ...
        (toolMappingManager.findAlternativeTool as jest.Mock).mockReturnValue({
        ...
        expect(() => 
          fallbackManager.executeWithFallback(toolCall, executeFn, ['filesystem.read_file_alt'])
        ).not.toThrow();
      });
    });
    ...
    describe('simplifyParameters', () => {
      it('should simplify complex parameters', () => {
        ...
        expect(() =>
          fallbackManager.executeWithFallback(toolCall, mockExecuteFn)
        ).not.toThrow();
      });
    });
  });
  ...
  describe('Fallback Strategies', () => {
    describe('alternative_tool strategy', () => {
      it('should try alternative tool when primary fails', async () => {
        ...
        (toolMappingManager.mapParameters as jest.Mock).mockReturnValue({ path: '/test.txt' });
        
        // First call fails, second succeeds with alternative
        mockExecuteFn
          .mockRejectedValueOnce(new Error('Tool not found'))
          .mockResolvedValueOnce({
            content: [{ type: 'text' as const, text: 'Success with alternative' }],
            isError: false,
          });
      ...
      it('should handle case when no alternative tool found', async () => {
        ...
        (toolMappingManager.findAlternativeTool as jest.Mock).mockReturnValue(null);
    ...
    describe('simplify_parameters strategy', () => {
      it('should simplify parameters when validation fails', async () => {
        ...
        mockExecuteFn
          ...
          .mockResolvedValueOnce({
            content: [{ type: 'text' as const, text: 'Success with simplified params' }],
            isError: false,
          });
    ...
    describe('retry_with_timeout strategy', () => {
      it('should increase timeout when timeout error occurs', async () => {
        ...
        mockExecuteFn
          ...
          .mockResolvedValueOnce({
            content: [{ type: 'text' as const, text: 'Success with increased timeout' }],
            isError: false,
          });
    ...
    describe('degrade_operation strategy', () => {
      it('should degrade complex operation to simpler one', async () => {
        ...
        mockExecuteFn
          ...
          .mockResolvedValueOnce({
            content: [{ type: 'text' as const, text: 'Degraded to list' }],
            isError: false,
          });
      ...
      it('should degrade analysis to simple read', async () => {
        ...
        mockExecuteFn
          ...
          .mockResolvedValueOnce({
            content: [{ type: 'text' as const, text: 'Degraded to read' }],
            isError: false,
          });
  ...
  describe('executeWithFallback', () => {
    it('should return successful result immediately', async () => {
      ...
      const successResult: ToolResult = {
        content: [{ type: 'text' as const, text: 'Operation successful' }],
        isError: false,
      };
    ...
    it('should handle error result from execute function', async () => {
      ...
      const errorResult: ToolResult = {
        content: [{ type: 'text' as const, text: 'Tool execution error' }],
        isError: true,
      };
    ...
    it('should apply strategies in priority order', async () => {
      ...
      const trackedStrategies = originalStrategies.map(strategy => ({
        ...
        condition: (error: any, toolCall: ToolCall, attemptCount: number) => {
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle strategy action throwing error', async () => {
      ...
      (manager as any).strategies = [failingStrategy];
    ...
    it('should handle empty arguments object', async () => {
      ...
      mockExecuteFn.mockResolvedValue({
        content: [{ type: 'text' as const, text: 'Success with empty args' }],
        isError: false,
      });
  ...
  describe('Integration Tests', () => {
    it('should chain multiple strategies', async () => {
      ...
      let callCount = 0;
      mockExecuteFn.mockImplementation(async (tc: ToolCall) => {
        callCount++;
