...
import {
  ...
  ProcessStatus,
  HealthStatus,
} from '../src/runtime/adapter-advanced';
...
describe('RuntimeAdapterRegistry', () => {
  beforeEach(() => {
    ...
    const factories = (RuntimeAdapterRegistry as any).factories;
    if (factories) {
      factories.clear();
    }
  });
  ...
  it('should register and get factory', () => {
    ...
    const mockAdapter = {
      start: async () => ({ id: 'test', status: 'running', config: {} } as ProcessInfo),
      stop: async () => {},
      status: async () => ({ running: true } as ProcessStatus),
    };
  ...
  it('should create adapter instance', () => {
    ...
    RuntimeAdapterRegistry.register('test-runtime', mockFactory);
    const config: ServiceConfig = { name: 'test', path: '/test' };
...
describe('BaseRuntimeAdapter', () => {
  class TestAdapter extends BaseRuntimeAdapter {
    private testProcesses = new Map<string, ProcessInfo>();
    ...
    async status(processId: string): Promise<ProcessStatus> {
  ...
  describe('restart', () => {
    ...
    it('should throw error when restarting non-existent process', async () => {
      await expect(adapter.restart('nonexistent-id')).rejects.toThrow(
        RuntimeAdapterError
      );
    });
  });
  ...
  describe('generateProcessId', () => {
    it('should generate unique process IDs', () => {
      const config: ServiceConfig = { name: 'test', path: '/test' };
      const id1 = (adapter as any).generateProcessId(config);
      const id2 = (adapter as any).generateProcessId(config);
  ...
  describe('validateConfig', () => {
    it('should throw error when name is missing', () => {
      const config = { path: '/test' } as ServiceConfig;

      expect(() => {
        (adapter as any).validateConfig(config);
      }).toThrow(RuntimeAdapterError);
    });

    it('should throw error when name is empty', () => {
      const config: ServiceConfig = { name: '', path: '/test' };
    ...
    it('should throw error when path is missing', () => {
      const config = { name: 'test' } as ServiceConfig;
    ...
    it('should throw error when path is empty', () => {
      const config: ServiceConfig = { name: 'test', path: '' };
...
describe('EnhancedRuntimeAdapter interface', () => {
  it('should define required methods', () => {
    const adapter: EnhancedRuntimeAdapter = {
      start: async () => ({ id: 'test', status: 'running', config: {} } as ProcessInfo),
      stop: async () => {},
      status: async () => ({ running: true } as ProcessStatus),
    };
  ...
  it('should support optional methods', () => {
    const adapter: EnhancedRuntimeAdapter = {
      ...
      stop: async () => {},
      status: async () => ({ running: true } as ProcessStatus),
      healthCheck: async () => ({ healthy: true, checks: [], score: 100 } as HealthStatus),
      logs: async function* () { yield 'test'; },
      restart: async () => ({ id: 'test', status: 'running', config: {} } as ProcessInfo),
      onStart: async () => {},
      onStarted: async () => {},
      onStop: async () => {},
      onStopped: async () => {},
