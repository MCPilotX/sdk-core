...
jest.mock('path', () => ({
    ...
    basename: jest.fn((filePath) => 'service'),
}));
...
describe('ServiceManager - Comprehensive Coverage Tests', () => {
    ...
    beforeEach(() => {
        ...
        mockPath.basename.mockImplementation((filePath) => {
            ...
            if (filePath.includes('service2'))
                return 'service2';
            if (filePath.includes('service3'))
                return 'service3';
            if (filePath.includes('node-service'))
                return 'node-service';
            if (filePath.includes('python-service'))
                return 'python-service';
            if (filePath.includes('docker-service'))
                return 'docker-service';
            if (filePath.includes('custom-service'))
                return 'custom-service';
            if (filePath.includes('env-service'))
                return 'env-service';
            if (filePath.includes('web-service'))
                return 'web-service';
    ...
    describe('restartService', () => {
        it('should restart a running service', async () => {
            ...
            const stopSpy = jest.spyOn(manager, 'stopService');
            const startSpy = jest.spyOn(manager, 'startService');
            // Restart the service
        ...
        it('should throw error when restarting non-existent service', async () => {
            await expect(manager.restartService('nonexistent')).rejects.toThrow('Service nonexistent is not installed');
        });
    });
    describe('uninstallService', () => {
        ...
        it('should throw error when uninstalling non-existent service', async () => {
            await expect(manager.uninstallService('nonexistent')).rejects.toThrow('Service nonexistent is not installed');
        });
    });
    ...
    describe('getServiceStats', () => {
        ...
        it('should throw error when getting stats for non-existent service', async () => {
            await expect(manager.getServiceStats('nonexistent')).rejects.toThrow('Service nonexistent not found');
        });
    });
    describe('spawnService - runtime coverage', () => {
        ...
        it('should spawn node service correctly', async () => {
            ...
            const process = manager.spawnService(service);
            expect(mockChildProcess.spawn).toHaveBeenCalledWith('node', ['index.js', '--port', '3000'], expect.objectContaining({
        ...
        it('should spawn python service correctly', async () => {
            ...
            manager.spawnService(service);
            expect(mockChildProcess.spawn).toHaveBeenCalledWith('python3', ['main.py'], expect.any(Object));
        });
        it('should spawn docker service correctly', async () => {
            ...
            manager.spawnService(service);
            expect(mockChildProcess.spawn).toHaveBeenCalledWith('docker', ['run', '-d', '--rm', '--name', 'mcp-docker-service', 'my-image:latest', '-p', '8080:80'], expect.any(Object));
        });
        it('should spawn service with default runtime', async () => {
            ...
            manager.spawnService(service);
            expect(mockChildProcess.spawn).toHaveBeenCalledWith('./custom-binary', ['--config', 'config.yaml'], expect.any(Object));
        });
    ...
    describe('detectEntryPoint', () => {
        ...
        it('should detect index.js as entry point', () => {
            ...
            const entryPoint = manager.detectEntryPoint('/path/to/service');
        ...
        it('should check all possible entry points in order', () => {
            ...
            mockFs.readdirSync.mockReturnValue([]);
            manager.detectEntryPoint('/path/to/service');
    ...
    describe('getProcessMemoryUsage', () => {
        it('should return undefined when process is undefined', () => {
            const memoryUsage = manager.getProcessMemoryUsage(undefined);
            expect(memoryUsage).toBeUndefined();
        });
        it('should return undefined when process has no pid', () => {
            const mockProcess = { exitCode: null };
            const memoryUsage = manager.getProcessMemoryUsage(mockProcess);
            expect(memoryUsage).toBeUndefined();
        });
    ...
    describe('edge cases and integration', () => {
        ...
        it('should handle service with custom runtime detection', async () => {
            ...
            const detectSpy = jest.spyOn(manager, 'detectEntryPoint');
...
//# sourceMappingURL=daemon-service-comprehensive.test.js.map