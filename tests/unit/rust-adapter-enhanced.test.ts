...
describe('RustAdapter Enhanced Tests', () => {
  ...
  describe('getSpawnArgs', () => {
    ...
    it('should handle single Rust file compilation', () => {
      const singleFileConfig: ServiceConfig = {
        ...mockConfig,
        entry: 'single.rs',
        runtimeConfig: {
          rust: {
  ...
  describe('setup', () => {
    it('should throw error when Rust/Cargo is not installed', async () => {
      ...
      await expect(adapter.setup(mockConfig)).rejects.toThrow(
        'Rust/Cargo is not installed or not in PATH'
      );
    });
    ...
    it('should build without release flag when not configured', async () => {
      const debugConfig: ServiceConfig = {
        ...mockConfig,
        runtimeConfig: {
          rust: {
            release: false,
    ...
    it('should skip tests when test flag is disabled', async () => {
      const noTestConfig: ServiceConfig = {
        ...mockConfig,
        runtimeConfig: {
          rust: {
            test: false,
      ...
      const testCalls = mockExecSync.mock.calls.filter((call: any[]) => 
        call[0] === 'cargo test'
      );
  ...
  describe('startService', () => {
    ...
    it('should start service with binary when found', async () => {
      ...
      expect(mockSpawn).toHaveBeenCalledWith(
        '/test/path/target/release/test-service',
        ['--verbose', '--release'],
        expect.any(Object)
      );
      expect(process).toBe(mockProcess);
  ...
  describe('edge cases', () => {
    ...
    it('should handle config without runtimeConfig', async () => {
      const simpleConfig: ServiceConfig = {
        name: 'simple-service',
        path: '/simple/path',
      };
