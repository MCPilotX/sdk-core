...
describe('ConfigManager - 简化测试', () => {
    ...
    describe('服务配置管理', () => {
        ...
        it('应该保存服务配置', () => {
            ...
            expect(mockFs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining(serviceName), expect.stringContaining(JSON.stringify(config, null, 2)));
        });
        it('应该列出所有服务', () => {
    ...
    describe('全局配置管理', () => {
        ...
        it('应该使用缓存加载全局配置', () => {
            ...
            // Set cache
            ConfigManager.globalConfigCache = config;
            const result = ConfigManager.getGlobalConfig();
    ...
    describe('Docker主机配置', () => {
        ...
        it('应该列出所有Docker主机', () => {
            ...
            mockPath.basename.mockImplementation((filename) => {
    ...
    describe('缓存管理', () => {
        it('应该缓存服务配置', () => {
            ...
            const config = { name: serviceName, path: '/path', runtime: 'node' };
            mockFs.readFileSync.mockReturnValue(JSON.stringify(config));
            mockFs.existsSync.mockReturnValue(true);
        ...
        it('应该清除所有缓存', () => {
            ...
            expect(ConfigManager.serviceConfigCache.size).toBe(0);
            expect(ConfigManager.globalConfigCache).toBeNull();
            expect(ConfigManager.servicesListCache).toBeNull();
            expect(ConfigManager.dockerHostsCache.size).toBe(0);
            expect(ConfigManager.runtimeProfilesCache.size).toBe(0);
        });
    });
    describe('错误处理', () => {
        it('应该处理JSON解析错误', () => {
...
//# sourceMappingURL=config-manager-simple.test.js.map