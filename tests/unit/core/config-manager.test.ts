...
describe('ConfigManager', () => {
  ...
  describe('缓存管理', () => {
    it('应该缓存服务配置', () => {
      ...
      const config = { name: serviceName, runtime: 'node' as const, path: '/path/to/service' };
    ...
    it('应该清除服务配置缓存', () => {
      ...
      // Add to cache
      (ConfigManager as any).serviceConfigCache.set(serviceName, {});
  ...
  describe('错误处理', () => {
    ...
    it('应该处理文件写入错误', () => {
      ...
      const config = { name: serviceName, path: '/path/to/service', runtime: 'node' as const };
