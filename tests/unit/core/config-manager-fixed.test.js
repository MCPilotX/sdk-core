...
describe('ConfigManager Fixed Test Suite', () => {
    let mockFs;
    let mockPath;
    beforeEach(() => {
        jest.clearAllMocks();
    ...
    describe('Service Configuration Management', () => {
        ...
        it('should update service detection with high confidence', () => {
            ...
            // Mock getServiceConfig to return config without runtime
            mockFs.readFileSync.mockImplementation((filePath) => {
                if (filePath.includes('config.json') && filePath.includes('global')) {
            ...
            expect(result.detectedRuntime).toBe('node');
            expect(result.detectionConfidence).toBe(0.9);
            expect(result.detectionSource).toBe('auto');
        });
    ...
    describe('Global Configuration', () => {
        ...
        it('should save global config', () => {
            ...
            // The actual path is /Users/liuyaping/.mcpilot/config.json
            // So we should check for config.json instead of global-config.json
...
//# sourceMappingURL=config-manager-fixed.test.js.map