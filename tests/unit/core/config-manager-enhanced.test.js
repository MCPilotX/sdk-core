...
describe('ConfigManager - Enhanced Test Suite', () => {
    ...
    describe('服务配置管理 - 完整测试', () => {
        it('应该保存服务配置并添加元数据', () => {
            ...
            expect(mockFs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining(serviceName), expect.stringContaining('lastDetectedAt'));
        });
        it('应该将相对路径转换为绝对路径', () => {
            const serviceName = 'test-service';
            const config = {
    ...
    describe('Docker主机配置 - 完整测试', () => {
        it('应该获取Docker主机配置', () => {
            ...
            expect(mockFs.readFileSync).toHaveBeenCalledWith(expect.stringContaining(`${hostName}.json`), 'utf-8');
        });
        ...
        it('应该删除Docker主机配置', () => {
            ...
            expect(mockFs.unlinkSync).toHaveBeenCalledWith(expect.stringContaining(`${hostName}.json`));
        });
    ...
    describe('运行时配置模板 - 完整测试', () => {
        ...
        it('应该返回null当运行时配置模板不存在', () => {
            const runtime = 'non-existent-runtime';
            mockFs.existsSync.mockReturnValue(false);
            const result = ConfigManager.getRuntimeProfile(runtime);
            expect(result).toBeNull();
        });
...
//# sourceMappingURL=config-manager-enhanced.test.js.map