...
describe('PythonAdapter Enhanced Tests', () => {
  ...
  describe('setup', () => {
    it('should create virtual environments directory when it does not exist', async () => {
      mockExistsSync.mockImplementation((path: string) => {
        if (path === VENVS_DIR) return false;
        return false;
      });
    ...
    it('should handle virtual environment creation warnings', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('Virtual environment creation warnings')
      );
    });
    ...
    it('should install dependencies from requirements.txt file', async () => {
      const configWithRequirements: ServiceConfig = {
        ...mockConfig,
        runtimeConfig: {
          python: {
            dependencies: 'requirements.txt' as any, // Type assertion to bypass TypeScript error
          },
        },
      };
      ...
      mockExistsSync.mockImplementation((path: string) => {
        ...
        if (path === '/test/venvs/test-python-service') return true;
        if (path === '/test/venvs/test-python-service/bin/python') return true;
        if (path === '/test/path/requirements.txt') return true;
        return false;
      });
      ...
      try {
        ...
        expect(mockExec).toHaveBeenCalledWith(
          expect.stringContaining('pip install -r "/test/path/requirements.txt"')
        );
        expect(mockLogger.info).toHaveBeenCalledWith(
          expect.stringContaining('Installing dependencies from requirements file')
        );
      } finally {
    ...
    it('should handle absolute path for requirements.txt', async () => {
      const configWithAbsolutePath: ServiceConfig = {
        ...
        runtimeConfig: {
          python: {
            dependencies: '/absolute/path/requirements.txt' as any, // Type assertion
          },
        },
      };
      ...
      mockExistsSync.mockImplementation((path: string) => {
        ...
        if (path === '/test/venvs/test-python-service') return true;
        if (path === '/test/venvs/test-python-service/bin/python') return true;
        if (path === '/absolute/path/requirements.txt') return true;
        return false;
      });
      ...
      expect(mockExec).toHaveBeenCalledWith(
        expect.stringContaining('pip install -r "/absolute/path/requirements.txt"')
      );
    });

    it('should warn when requirements.txt file not found', async () => {
      const configWithMissingRequirements: ServiceConfig = {
        ...mockConfig,
        runtimeConfig: {
          python: {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('Requirements file not found')
      );
    });
    ...
    it('should handle dependency installation warnings', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('Installation warnings')
      );
    });
    ...
    it('should handle individual dependency installation failure', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining('Failed to install dependency flask')
      );
    ...
    it('should skip dependency installation when no dependencies configured', async () => {
      ...
      const pipInstallCalls = mockExec.mock.calls.filter((call: any[]) => 
    ...
    it('should handle virtual environment creation failure', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining('Failed to setup Python environment for test-python-service'),
        expect.any(Object) // Second parameter is the error object with stack
      );
    }, 10000);
  });
  ...
  describe('edge cases', () => {
    ...
    it('should handle config with empty python config', async () => {
      const configWithEmptyPythonConfig: ServiceConfig = {
      ...
      mockExistsSync.mockImplementation((path: string) => {
        if (path === VENVS_DIR) return true;
        if (path === '/test/venvs/empty-python-config-service') return true;
        if (path === '/test/venvs/empty-python-config-service/bin/python') return true;
        return false;
      });
    ...
    it('should handle config without python config in runtimeConfig', async () => {
      const configWithoutPythonConfig: ServiceConfig = {
      ...
      mockExistsSync.mockImplementation((path: string) => {
        if (path === VENVS_DIR) return true;
        if (path === '/test/venvs/no-python-config-service') return true;
        if (path === '/test/venvs/no-python-config-service/bin/python') return true;
        return false;
      });
    ...
    it('should handle partial virtual environment existence', async () => {
      ...
      mockExistsSync.mockImplementation((path: string) => {
        if (path === VENVS_DIR) return true;
        if (path === '/test/venvs/test-python-service') return true;
        if (path === '/test/venvs/test-python-service/bin/python') return false;
        return false;
      });
