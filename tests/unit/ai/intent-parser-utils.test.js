...
describe('IntentParserUtils', () => {
    ...
    beforeEach(() => {
        ...
        IntentParserSelector.mockImplementation(() => mockSelector);
        RuleBasedParser.mockImplementation(() => mockRuleBasedParser);
        HybridAIParser.mockImplementation(() => mockHybridAIParser);
    });
    ...
    describe('edge cases', () => {
        it('should handle null or undefined AI config', () => {
            ...
            // Test with null
            const parser2 = IntentParserUtils.createAIParser(null);
...
describe('IntentParserExamples', () => {
    ...
    it('should have diverse example types', () => {
        ...
        const fileQueries = IntentParserExamples.filter(q => q.toLowerCase().includes('file') || q.toLowerCase().includes('directory'));
        const systemQueries = IntentParserExamples.filter(q => q.toLowerCase().includes('start') || q.toLowerCase().includes('stop') || q.toLowerCase().includes('service'));
        const networkQueries = IntentParserExamples.filter(q => q.toLowerCase().includes('ping'));
...
//# sourceMappingURL=intent-parser-utils.test.js.map