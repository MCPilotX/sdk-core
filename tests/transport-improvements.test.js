...
class TestableTransport extends BaseTransport {
    ...
    testLooksLikeJsonChunk(text) {
        return this.looksLikeJsonChunk(text);
    }
    testLooksLikeJsonMessage(message) {
        return this.looksLikeJsonMessage(message);
    }
    testIsLogMessage(message) {
        return this.isLogMessage(message);
    }
...
describe('Transport Improvements', () => {
    ...
    describe('Buffer Management', () => {
        test('should clear buffer correctly', () => {
            ...
            transport.jsonBuffer = 'test data';
            transport.bufferTimeout = setTimeout(() => { }, 1000);
            transport.lastBufferUpdate = Date.now();
            transport.testClearBuffer();
            expect(transport.jsonBuffer).toBe('');
            expect(transport.bufferTimeout).toBeNull();
            expect(transport.lastBufferUpdate).toBe(0);
        });
        test('should try parse buffer', () => {
            // Test with valid JSON
            transport.jsonBuffer = '{"jsonrpc": "2.0", "id": 1}';
            const result1 = transport.testTryParseBuffer();
            expect(result1).toEqual({ jsonrpc: '2.0', id: 1 });
            // Test with invalid JSON
            transport.jsonBuffer = '{ invalid json }';
            const result2 = transport.testTryParseBuffer();
            expect(result2).toBeNull();
            // Test with empty buffer
            transport.jsonBuffer = '';
            const result3 = transport.testTryParseBuffer();
            expect(result3).toBeNull();
        });
    });
});
//# sourceMappingURL=transport-improvements.test.js.map