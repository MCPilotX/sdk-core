...
describe('Runtime Coverage Tests', () => {
    describe('NodeAdapter', () => {
        ...
        it('should call method successfully', async () => {
            ...
            adapter.pendingRequests.set = jest.fn();
            const promise = adapter.call('testMethod', { param: 'value' });
            expect(adapter.pendingRequests.set).toHaveBeenCalled();
            expect(adapter.process.stdin.write).toHaveBeenCalled();
        });
    ...
    describe('PythonAdapter', () => {
        ...
        it.skip('should create virtual environment when it does not exist', async () => {
            ...
            const mockPromisify = jest.fn().mockReturnValue(jest.fn().mockResolvedValue({
                stdout: '',
    ...
    describe('PythonRuntimeAdapter', () => {
        ...
        it('should call method successfully', async () => {
            // Mock process and pending requests
            const mockResolve = jest.fn();
            pythonAdapter.process = {
                stdin: { write: jest.fn() }
            };
            pythonAdapter.pendingRequests = new Map();
            pythonAdapter.pendingRequests.set = jest.fn();
            const promise = pythonAdapter.call('testMethod', { param: 'value' });
            expect(pythonAdapter.pendingRequests.set).toHaveBeenCalled();
            expect(pythonAdapter.process.stdin.write).toHaveBeenCalled();
        });
        it('should throw error when calling method without process', async () => {
            pythonAdapter.process = null;
        ...
        it('should stop successfully', () => {
            ...
            pythonAdapter.stop();
            expect(mockKill).toHaveBeenCalled();
            expect(pythonAdapter.process).toBeNull();
        });
        it('should check if running', () => {
            pythonAdapter.process = {};
...
//# sourceMappingURL=runtime-coverage.test.js.map