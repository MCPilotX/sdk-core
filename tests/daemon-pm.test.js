...
describe('ProcessManager', () => {
    ...
    beforeEach(() => {
        ...
        mockFs = fs;
        mockPath = path;
        mockNodeAdapter = NodeAdapter;
        mockPythonAdapter = PythonAdapter;
        mockDockerAdapter = DockerAdapter;
    ...
    describe('tool discovery', () => {
        it('should discover tools from running service', async () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Discovered 2 tools from test-service-node'));
            // Clean up
        ...
        it('should handle tool discovery failure', async () => {
            ...
            expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to discover tools for test-service-node'));
    ...
    describe('service calling', () => {
        it('should call service method', async () => {
            ...
            mockNodeAdapter.mockImplementation(() => mockAdapterInstance);
            const result = await processManager.callService('test-service-node', 'testMethod', { param1: 'value1' });
        ...
        it('should start service if not running when calling', async () => {
            ...
            const result = await processManager.callService('test-service-node', 'testMethod', {});
    ...
    describe('service stopping', () => {
        it('should stop a running service', async () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Service test-service-node stopped'));
            consoleLogSpy.mockRestore();
        });
...
//# sourceMappingURL=daemon-pm.test.js.map