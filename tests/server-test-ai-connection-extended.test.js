...
describe('DaemonServer.testAIConnection - Extended Tests', () => {
    ...
    describe('network and error handling', () => {
        ...
        it('should handle timeout scenarios', async () => {
            // Mock slow response
            global.fetch.mockImplementation(() => new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), 100)));
            const mockOrchestrator = {
                getConfig: () => ({
                    ai: {
                        provider: 'openai',
    ...
    describe('edge cases', () => {
        ...
        it('should handle null orchestrator', async () => {
            const result = await DaemonServer.testAIConnection(null);
            expect(result.success).toBe(false);
            expect(result.error).toContain('AI configuration not set');
        });
        it('should handle undefined orchestrator', async () => {
            const result = await DaemonServer.testAIConnection(undefined);
            expect(result.success).toBe(false);
            expect(result.error).toContain('AI configuration not set');
        });
    ...
    describe('performance and reliability', () => {
        it('should handle rapid successive calls', async () => {
            ...
            const results = await Promise.all([
                DaemonServer.testAIConnection(mockOrchestrator),
                DaemonServer.testAIConnection(mockOrchestrator),
                DaemonServer.testAIConnection(mockOrchestrator),
            ]);
            // All should succeed
        ...
        it('should not leak memory on repeated errors', async () => {
            ...
            global.fetch.mockRejectedValue(new Error('Consistent error'));
            const mockOrchestrator = {
                getConfig: () => ({
                    ai: {
                        provider: 'openai',
...
//# sourceMappingURL=server-test-ai-connection-extended.test.js.map