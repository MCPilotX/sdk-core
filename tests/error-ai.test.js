...
describe('AIErrorHandler', () => {
    ...
    describe('handleError', () => {
        it('should handle config error with provider-specific suggestions', () => {
            ...
            expect(mockConsoleLog.mock.calls.some(call => call[0].includes('❌ Configuration error'))).toBe(true);
        });
        it('should handle connection error with network suggestions', () => {
            ...
            expect(mockConsoleLog.mock.calls.some(call => call[0].includes('🔧 Repair suggestions:'))).toBe(true);
        });
        ...
        it('should provide Ollama-specific suggestions for connection errors', () => {
            ...
            expect(mockConsoleLog.mock.calls.some(call => typeof call[0] === 'string' && call[0].includes('Ollama service'))).toBe(true);
        });
    });
    describe('getSuggestions', () => {
        it('should return config suggestions for config errors', () => {
            ...
            expect(mockConsoleLog.mock.calls.some(call => typeof call[0] === 'string' && call[0].includes('Check configuration file'))).toBe(true);
        });
        it('should return connection suggestions for connection errors', () => {
            ...
            expect(mockConsoleLog.mock.calls.some(call => typeof call[0] === 'string' && call[0].includes('Check network connection'))).toBe(true);
        });
    });
});
//# sourceMappingURL=error-ai.test.js.map