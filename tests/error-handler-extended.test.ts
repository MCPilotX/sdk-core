...
describe('Error Handler Extended Tests', () => {
  describe('IntentOrchError Methods', () => {
    it('should convert error to JSON format', () => {
      ...
      const error = new IntentOrchError(
        ErrorCode.CONFIG_INVALID,
        'Invalid configuration',
        ErrorSeverity.HIGH,
        { configFile: 'test.json', lineNumber: 42 },
        [{ title: 'Fix config', description: 'Check format', steps: ['Step 1'] }],
        cause
      );
    ...
    it('should convert IntentOrchError cause to JSON', () => {
      const innerError = new IntentOrchError(
    ...
    it('should get detailed error information', () => {
      const error = new IntentOrchError(
        ...
        'Invalid configuration',
        ErrorSeverity.HIGH,
        { file: 'config.json', reason: 'malformed' },
        [
          {
  ...
  describe('ErrorFactory', () => {
    ...
    it('should create runtime detection failed error', () => {
      ...
      const error = ErrorFactory.runtimeDetectionFailed(
        '/path/to/project',
        { attempt: 1 },
        cause
      );
    ...
    it('should create process start failed error', () => {
      ...
      const error = ErrorFactory.processStartFailed(
        'web-server',
        { port: 3000 },
        cause
      );
  ...
  describe('ErrorHandler', () => {
    ...
    it('should execute function and handle error', async () => {
      ...
      await expect(
        errorHandler.execute('test operation', mockFn, { attempt: 1 })
      ).rejects.toThrow();
    ...
    it('should execute function and handle non-Error rejection', async () => {
      ...
      await expect(
        errorHandler.execute('test operation', mockFn)
      ).rejects.toThrow();
    ...
    it('should execute function and preserve MCPilotError', async () => {
      ...
      const mcError = new MCPilotError(
        ErrorCode.NETWORK_ERROR,
        'Network failure'
      );
      ...
      await expect(
        errorHandler.execute('network operation', mockFn)
      ).rejects.toThrow(mcError);
  ...
  describe('ConsoleErrorHandler', () => {
    ...
    it('should handle error with different severity colors', async () => {
      const lowError = new MCPilotError(
        ErrorCode.NOT_IMPLEMENTED,
        'Feature not implemented',
        ErrorSeverity.LOW
      );
      
      const mediumError = new MCPilotError(
        ErrorCode.NETWORK_ERROR,
        'Network issue',
        ErrorSeverity.MEDIUM
      );
      
      const highError = new MCPilotError(
        ErrorCode.CONFIG_INVALID,
        'Critical config error',
        ErrorSeverity.HIGH
      );
      
      const criticalError = new MCPilotError(
  ...
  describe('RetryErrorHandler', () => {
    ...
    it('should execute successfully on first attempt', async () => {
      ...
      const result = await RetryErrorHandler.withRetry(
        'test operation',
        mockFn,
        { maxAttempts: 3, backoff: 'fixed', baseDelay: 100 }
      );
    ...
    it('should retry on failure with linear backoff', async () => {
      ...
      const result = await RetryErrorHandler.withRetry(
        'test operation',
        mockFn,
        { maxAttempts: 3, backoff: 'linear', baseDelay: 100 }
      );
    ...
    it('should apply maximum delay limit', async () => {
      ...
      await expect(
        RetryErrorHandler.withRetry(
          'test operation',
          mockFn,
          { maxAttempts: 3, backoff: 'exponential', baseDelay: 1000, maxDelay: 1500 }
        )
    ...
    it('should throw error after max attempts', async () => {
      ...
      await expect(
        RetryErrorHandler.withRetry(
          'test operation',
          mockFn,
          { maxAttempts: 2, backoff: 'fixed', baseDelay: 10 }
        )
    ...
    it('should handle non-Error rejection', async () => {
      ...
      try {
        await RetryErrorHandler.withRetry(
          'test operation',
          mockFn,
          { maxAttempts: 1, backoff: 'fixed', baseDelay: 0 }
        );
      } catch (error) {
        // Expected to throw
      }
