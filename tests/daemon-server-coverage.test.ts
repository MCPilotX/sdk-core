...
describe('DaemonServer Coverage Tests', () => {
  ...
  describe('constructor', () => {
    ...
    it('should initialize ProcessManager and Orchestrator', () => {
      ...
      const pm = (daemonServer as any).pm;
      const orchestrator = (daemonServer as any).orchestrator;
  ...
  describe('setupRoutes', () => {
    it('should setup routes', () => {
      ...
      const app = (daemonServer as any).app;
  ...
  describe('start', () => {
    it('should start the server', async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        'Starting MCPilot Daemon on http://localhost:8082...'
      );
  ...
  describe('route handlers (simulated)', () => {
    ...
    it('should handle start-service command', async () => {
      ...
      expect(mockContext.json).toHaveBeenCalledWith(
        { status: 'ok', message: 'Service test-service started.' }
      );
    ...
    it('should handle stop-service command', async () => {
      ...
      expect(mockContext.json).toHaveBeenCalledWith(
        { status: 'ok', message: 'Service test-service stopped.' }
      );
    ...
    it('should handle get-config command', async () => {
      const mockContext = {
        req: {
          json: jest.fn().mockResolvedValue({ command: 'get-config' } as any),
        },
        json: jest.fn(),
      };
    ...
    it('should handle get-ai-config command', async () => {
      const mockContext = {
        req: {
          json: jest.fn().mockResolvedValue({ command: 'get-ai-config' } as any),
        },
        json: jest.fn(),
      };
    ...
    it('should handle unknown command', async () => {
      ...
      expect(mockContext.json).toHaveBeenCalledWith(
        { status: 'error', message: 'Unknown command: unknown-command' },
        404
      );
    ...
    it('should handle errors in request processing', async () => {
      ...
      expect(mockContext.json).toHaveBeenCalledWith(
        { status: 'error', message: 'JSON parsing error' },
        500
      );
    ...
    it('should handle errors in start-service command', async () => {
      ...
      expect(mockContext.json).toHaveBeenCalledWith(
        { status: 'error', message: 'Failed to start service' },
        500
      );
      expect(logger.error).toHaveBeenCalled();
    });
    ...
    it('should handle errors in stop-service command', async () => {
      ...
      expect(mockContext.json).toHaveBeenCalledWith(
        { status: 'error', message: 'Failed to stop service' },
        500
      );
      expect(logger.error).toHaveBeenCalled();
    });
