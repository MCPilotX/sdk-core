/**
 * Comprehensive test suite for DockerAdapter
 */
// @ts-nocheck
...
describe('DockerAdapter', () => {
  ...
  describe('constructor', () => {
    it('should create instance with empty container name', () => {
      ...
      expect((dockerAdapter as any).containerName).toBe('');
      expect((dockerAdapter as any).process).toBeNull();
    });
  });
  ...
  describe('setup', () => {
    ...
    it('should build image from Dockerfile if specified', async () => {
      ...
      // Mock path methods
      const mockPathJoin = jest.spyOn(require('path'), 'join');
      mockPathJoin.mockReturnValue('/project/path/Dockerfile');
      
      const mockPathDirname = jest.spyOn(require('path'), 'dirname');
      ...
      expect(mockExecSync).toHaveBeenCalledWith(
        'docker build -t test-service -f /project/path/Dockerfile /project/path',
        { stdio: 'inherit', cwd: '/project/path' }
      );
    ...
    it('should use custom build context if specified', async () => {
      ...
      expect(mockExecSync).toHaveBeenCalledWith(
        'docker build -t test-service -f /project/path/Dockerfile /build/context',
        { stdio: 'inherit', cwd: '/project/path' }
      );
  ...
  describe('startContainer', () => {
    it('should start container with correct arguments', async () => {
      ...
      expect(require('child_process').spawn).toHaveBeenCalledWith(
        'docker',
        expect.arrayContaining(['run', '-d', '--rm', '--name', expect.any(String), 'nginx:latest']),
        {
      ...
      expect((dockerAdapter as any).process).toBe(mockProcess);
    });
  ...
  describe('stopContainer', () => {
    it('should stop container when process exists', async () => {
      (dockerAdapter as any).process = mockProcess;
      (dockerAdapter as any).containerName = 'test-container-123';
    ...
    it('should do nothing when no process exists', async () => {
      (dockerAdapter as any).process = null;
  ...
  describe('getContainerStatus', () => {
    it('should return "not_created" when container name is empty', async () => {
      (dockerAdapter as any).containerName = '';
    ...
    it('should return "running" when container is up', async () => {
      ...
      expect(mockExecSync).toHaveBeenCalledWith(
        'docker ps -a --filter "name=test-container-123" --format "{{.Status}}"',
        { encoding: 'utf-8' }
      );
    });
  ...
  describe('getContainerLogs', () => {
    ...
    it('should return container logs', async () => {
      ...
      expect(mockExecSync).toHaveBeenCalledWith(
        'docker logs --tail 100 test-container-123',
        { encoding: 'utf-8' }
      );
    });
    ...
    it('should use default tail value', async () => {
      ...
      expect(mockExecSync).toHaveBeenCalledWith(
        'docker logs --tail 50 test-container-123',
        { encoding: 'utf-8' }
      );
    });
  ...
  describe('integration scenarios', () => {
    it('should handle complete container lifecycle', async () => {
      ...
      // Check status
      mockExecSync.mockReturnValue('Up 1 minute');
      const status = await dockerAdapter.getContainerStatus();
      expect(status).toBe('running');
      
      // Get logs
      mockExecSync.mockReturnValue('nginx logs');
      const logs = await dockerAdapter.getContainerLogs();
      expect(logs).toBe('nginx logs');
