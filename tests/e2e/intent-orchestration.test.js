...
// Mock tool executors
const mockExecutors = {
    'filesystem.list_directory': async (args) => ({
        content: [{ type: 'text', text: `Listing files in ${args.path}` }],
        isError: false,
    }),
    'filesystem.read_file': async (args) => ({
        content: [{ type: 'text', text: `Reading file ${args.path}` }],
        isError: false,
    }),
    'filesystem.write_file': async (args) => ({
        content: [{ type: 'text', text: `Writing to file ${args.path}` }],
        isError: false,
    }),
    'network.ping_host': async (args) => ({
        content: [{ type: 'text', text: `Pinging ${args.host}` }],
        isError: false,
    }),
    'service_manager.start_service': async (args) => ({
        content: [{ type: 'text', text: `Starting service ${args.name}` }],
        isError: false,
    }),
};
describe('Intent Orchestration End-to-End Tests', () => {
    ...
    beforeEach(() => {
        ...
        mockTools.forEach(tool => {
            toolRegistry.registerTool(tool, mockExecutors[tool.name], 'mock-server', 'Mock Server');
        });
    });
    afterEach(() => {
    ...
    describe('Fallback Mechanisms', () => {
        it('should try alternative tools when primary fails', async () => {
            ...
            }, failingExecutor, 'failing-server', 'Failing Server');
            ...
            const executeFn = async (tc) => {
                if (tc.name === 'filesystem.list_files') {
                    return {
                        content: [{ type: 'text', text: 'Alternative tool worked' }],
                        isError: false,
                    };
                }
                return await failingExecutor();
            };
            const result = await defaultFallbackManager.executeWithFallback(toolCall, executeFn, availableTools);
        ...
        it('should simplify parameters when validation fails', async () => {
            ...
            const mockExecutor = async (args) => {
                ...
                return {
                    content: [{ type: 'text', text: 'Write successful with simplified parameters' }],
                    isError: false,
                };
            };
            const executeFn = async (tc) => {
    ...
    describe('Error Handling and Degradation', () => {
        it('should degrade complex operations to simpler ones', async () => {
            ...
            const mockExecutor = async (args) => {
                ...
                return {
                    content: [{ type: 'text', text: 'Degraded operation successful' }],
                    isError: false,
                };
            };
            const executeFn = async (tc) => {
...
//# sourceMappingURL=intent-orchestration.test.js.map