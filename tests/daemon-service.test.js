...
describe('ServiceManager - Simple Tests', () => {
    ...
    describe('error handling', () => {
        it('should throw error when installing non-existent service', async () => {
            mockFs.existsSync.mockReturnValue(false);
            await expect(manager.installService('/nonexistent/path')).rejects.toThrow('Service path not found: /nonexistent/path');
        });
        it('should throw error when installing already installed service', async () => {
            ...
            await manager.installService('/path/to/service', 'test-service');
            await expect(manager.installService('/path/to/service', 'test-service')).rejects.toThrow('Service test-service is already installed');
        });
        it('should throw error when starting non-installed service', async () => {
            await expect(manager.startService('nonexistent')).rejects.toThrow('Service nonexistent is not installed');
        });
        it('should handle error when starting service fails', async () => {
            ...
            await manager.installService('/path/to/service', 'test-service');
            await expect(manager.startService('test-service')).rejects.toThrow('Failed to start service test-service: Failed to spawn process');
        });
        it('should throw error when stopping non-installed service', async () => {
            await expect(manager.stopService('nonexistent')).rejects.toThrow('Service nonexistent is not installed');
        });
    });
...
//# sourceMappingURL=daemon-service.test.js.map