...
describe('Orchestrator', () => {
  ...
  beforeEach(() => {
    ...
    mockFs = fs as any;
    mockProcessManager = ProcessManager as any;
    mockHybridAIParser = HybridAIParser as any;
    mockConfigValidator = ConfigValidator as any;
    
    // Mock ProcessManager instance
    const mockPmInstance = {
      getRunningServices: jest.fn<any>().mockReturnValue(['test-service']),
      getServiceTools: jest.fn<any>().mockReturnValue([
        { name: 'list_files', description: 'List files in directory' },
        { name: 'read_file', description: 'Read file contents' },
      ]),
      callService: jest.fn<any>().mockResolvedValue({ success: true, data: 'test result' }),
    };
    ...
    mockConfigValidator.validate.mockImplementation((config: any) => config);
  ...
  describe('executeQuery', () => {
    ...
    it('should execute query successfully', async () => {
      ...
      expect(mockPmInstance.callService).toHaveBeenCalledWith(
        'test-service',
        'list_files',
        { path: '/tmp' }
      );
      ...
      expect(mockIntentInstance.parse).toHaveBeenCalledWith(
        'list files in /tmp',
        { availableTools: ['test-service:list_files', 'test-service:read_file'] }
      );
    });
