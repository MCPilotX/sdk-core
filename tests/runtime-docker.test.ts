...
import { DockerAdapter, AdapterOptions } from '../src/runtime/docker';
...
describe('DockerAdapter', () => {
  ...
  describe('constructor', () => {
    it('should create adapter with options', () => {
      ...
      expect((adapter as any).options).toEqual(options);
      expect((adapter as any).process).toBeNull();
      expect((adapter as any).requestId).toBe(0);
    ...
    it('should create adapter without environment variables', () => {
      ...
      expect((adapter as any).options).toEqual(options);
      expect((adapter as any).options.env).toBeUndefined();
    });
  });
  ...
  describe('start', () => {
    it('should start Docker container successfully', async () => {
      ...
      expect((adapter as any).process).toBe(mockProcess);
    });
  ...
  describe('call', () => {
    it('should send JSON-RPC request and receive response', async () => {
      ...
      expect(mockStdin.write).toHaveBeenCalledWith(
        '{"jsonrpc":"2.0","id":1,"method":"testMethod","params":{"param":"value"}}\n'
      );

      // Verify pending request was registered
      expect((adapter as any).pendingRequests.size).toBe(1);
      expect((adapter as any).pendingRequests.has(1)).toBe(true);
    ...
    it('should throw error when process is not running', async () => {
      ...
      await expect(adapter.call('testMethod')).rejects.toThrow(
        'Docker service test-service is not running.'
      );
    });
  ...
  describe('stop', () => {
    it('should stop running container', () => {
      ...
      const adapter = new DockerAdapter(options);
      (adapter as any).process = mockProcess;
  ...
  describe('stdout data handling', () => {
    it('should resolve pending request when matching ID is received', async () => {
      ...
      (adapter as any).pendingRequests.set(42, resolver);
      ...
      expect((adapter as any).pendingRequests.has(42)).toBe(false);
    });
    ...
    it('should ignore JSON without ID field', async () => {
      ...
      expect((adapter as any).pendingRequests.has(42)).toBe(true);
    });
